...
```bash
lamarr-sm$ sam build --use-container
```
...
```bash
lamarr-sm$ sam logs -n StockCheckerFunction --stack-name lamarr-sm --tail
```
...
```bash
lamarr-sm$ pip install pytest pytest-mock --user
lamarr-sm$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name lamarr-sm
```
